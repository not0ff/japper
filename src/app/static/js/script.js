...
async function showUserList(element) {
    ...
    modal.querySelector('.modal-body').innerHTML = result.userList;
...
async function getAllNotifications(unread_only = true, autoread = true) {
    ...
    if (Object.keys(result.notifications).length !== 0) {
        ...
        const infoPopover = bootstrap.Popover.getOrCreateInstance(notificationsBtn, {
            container: 'body',
            trigger: 'focus',
...
async function showUnreadCount() {
    ...
    const notificationsBtnLink = document.getElementById('notificationsBtn').querySelector('span');
