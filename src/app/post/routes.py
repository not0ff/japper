...
def fetch_post() -> ResponseReturnValue:
    ...
    return jsonify(
        {
            "status": "ok",
            "result": {"post": render_template("post/post.html", post=post)},
        }
    ), 200
...
def like_post() -> ResponseReturnValue:
    ...
    if current_user != post.author:
        add_notification(
            user=post.author, content=f"{current_user.username} gave you aura!"
        )
    db.session.commit()
    ...
    return jsonify(
        {
            ...
            "result": {
                "userList": render_template(
                    "components/user_list.html", users=likes_users
                )
            },
        }
    ), 200
...
def create_post() -> ResponseReturnValue:
    ...
    if form.validate_on_submit():
        post: Post = Post(user_id=current_user.id, content=form.content.data)
        db.session.add(post)
        for follower in current_user.followers:
            add_notification(
                user=follower, content=f"{current_user.username} has made a new post!"
            )
