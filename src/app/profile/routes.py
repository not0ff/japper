...
from app.utils import (flash_form_errors, get_profile, optimize_img,
                       redirect_to_referrer, add_notification)
...
def profile_page(username: str) -> ResponseReturnValue:
    ...
    return render_template('profile/profile.html', user=user, posts=posts, profile_form=profile_form, follow_profile_form=follow_profile_form)
...
def follow_profile() -> ResponseReturnValue:
    ...
    if form.validate_on_submit():
        user: User = User.query.filter_by(
            id=form.profile_id.data).first_or_404()
    ...
    return redirect_to_referrer(fallback=url_for('profile.profile_page', username=current_user.username))
