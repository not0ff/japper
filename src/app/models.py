...
class User(UserMixin, db.Model):  # type: ignore
    ...
    likes: Mapped[list["Like"]] = relationship(
        "Like", backref=backref("user"), cascade="all, delete-orphan"
    )
    ...
    notified: Mapped["Notification"] = relationship(
        "Notification",
        backref=backref("issuer"),
        foreign_keys="Notification.issuer_id",
        cascade="all, delete-orphan",
    )
...
class Notification(db.Model):  # type: ignore
    ...
    timestamp: Mapped[float] = mapped_column(
        default=lambda: datetime.now(UTC).timestamp(), index=True
    )
